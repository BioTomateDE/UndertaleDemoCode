...
if global.mnfight = 2 then
    ...
    if global.monster[0]+global.monster[1]+global.monster[2]=3 then global.firingrate=global.firingrate*1.5
    if global.monster[0]+global.monster[1]+global.monster[2]=2 then global.firingrate=global.firingrate*1.25
    if mycommand >=0 and mycommand <=40 then
       {
       if (global.monster[0]+global.monster[1]+global.monster[2])>1 then gen = instance_create(x,y,obj_1sidegen)
       else gen=instance_create(global.idealborder[1]-40,global.idealborder[3]-40,blt_leapfrog)
